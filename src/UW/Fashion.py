...
class Fashion:
    cityMapList = [
        ...
        {
            "name": "南太平洋",
            "cities": ["pinjara", "kakatuwah", "芬瑟岛", "皮里港", "霍巴特"],
        },
        {
            "name": "大西洋&西地中海",
            "cities": [
    ...
    def get_hour(self):
        ...
        target = (
            datetime(2023, 1, 24, 0, 0, 0)
            + timedelta(hours=1)
            + (timedelta(hours=1) if self.isDst() else timedelta(hours=0))
        )  # 假设目标时间为GMT+9
        # 获取当前时间aden
    ...
    def getFashionsByMap(self, name, hours):
        map = next(
            (map_item for map_item in self.mapList if map_item["name"] == name), None
        )
        if not map:
    ...
    def getExtraMinutesByCity(self, cityName):
        ...
        if mapName == None:
            return 0
        map = next(
            (map_item for map_item in self.mapList if map_item["name"] == mapName), None
        )
        if not map:
            return 0
